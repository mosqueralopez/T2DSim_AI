...
class CGMOHSUSimStateSpaceModel_T2DOGTT(nn.Module):
    def __init__(self, n_feat=None, scale_dx=1.0,  init_small=True):
        ...
        if init_small:
            ...
            networks = {'C1':self.net_dC1,'C2':self.net_dC2,'Gc':self.net_dGc,
                        'Ge':self.net_dGe,'Ie':self.net_dIe,'I':self.net_dI}
    ...
    def forward(self, in_x, in_u):
        ...
        C1, C2, Gc, Ge, Ie, I = in_x[..., [0]],in_x[..., [1]],in_x[..., [2]],in_x[..., [3]],in_x[..., [4]],in_x[..., [5]]
...
class NeuralOGTT:
    ...
    def setup_simulator(self):
        ...
        ss_ogtt_model.load_state_dict(torch.load(Path(__file__).parent /"models/OGTT_productionModel_6compartments.pt"))

        self.nn_solution = ForwardEulerSimulator(ss_model = ss_ogtt_model, ss_DT_model=None, ts=self.ts)

    def prepare_data(self,df_scenario):
        _, u_ogtt = scaler_OGTT(df_scenario[states].values, df_scenario[inputs_OGTT].values)
        ...
        x0_est = torch.tensor(df_scenario.loc[0,states].astype(float).values.reshape(1,-1),dtype=torch.float32).to(self.device)
        u_ogtt = torch.tensor(u_ogtt[[0],batch_idx.T],dtype=torch.float32).to(self.device)
    ...
    def simulate(self,df_scenario_original):
        ...
        with torch.no_grad():
            ...
            df_scenario[states ] = scaler_inverse(x_sim[:,0,:].to('cpu').detach().numpy())
