...
class ForwardEulerSimulator(nn.Module):
    ...
    def __init__(self, ss_model,ss_DT_model=None, ts=1.0, states_info = {'C1':0,'C2':1,'Gc':2,'Ge':3,'Ie':4,'I':5}):
    ...
    def forward(self, x0_batch: torch.Tensor, u_batch_ogtt: torch.Tensor,u_batch_pop=None,is_DT=False) -> torch.Tensor:
    ...
    def forward_sobol(self, x0_batch: torch.Tensor, u_batch: torch.Tensor, weights: list) -> torch.Tensor:
        """ Multi-step simulation over (mini)batches
        ...
        for u_step in u_batch.split(1):  # i in range(seq_len):
            ...
            dx = self.ss_model(x_step, u_step) * torch.tensor(weights, dtype=torch.float32)
            x_step = x_step + self.ts*dx
